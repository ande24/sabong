...
import { collection, doc, onSnapshot, updateDoc, setDoc, Timestamp } from "firebase/firestore";
...
export default function AdminControls() {
    ...
    async function updateAdminConfig(updates: Array<[string, any]>): Promise<void> {
        const configRef = doc(collection(db, "admin"), "config");
        const updateObj: Record<string, any> = {};
        updates.forEach(([field, value]) => {
            updateObj[field] = value;
        });
        await updateDoc(configRef, updateObj);
    }



    const addPettyCash = async (e: React.FormEvent) => {
        e.preventDefault();
        const amount = parseFloat(pettyCashAmount);
        if (!isNaN(amount)) {
            await updateAdminConfig([["petty_cash", adminConfig.petty_cash + amount]]);
            setPettyCashAmount("");
    ...
    const updateCommission = async (e: React.FormEvent) => {
        e.preventDefault();
        const percentage = parseFloat(newCommission);
        if (!isNaN(percentage) && percentage >= 0 && percentage <= 100) {
            await updateAdminConfig([["commission_pct", percentage]]);
            setNewCommission("");
    ...
    const handleStartNewFight = async () => {
        console.log("Starting new fight...");
        console.log("Current fight number: ", adminConfig.current_fight);
        console.log("Current game number: ", adminConfig.current_game);
        const now = new Date();
        ...
        const fightData = {
            id: fightId,
            fight_number: adminConfig.current_fight + 1,
            game_number: adminConfig.current_game,
            meron_odds: 1,
            meron_total: 0,
            wala_odds: 1,
            wala_total: 0,
        ...
        await updateAdminConfig([["current_fight", adminConfig.current_fight + 1], ["meron_open", false], ["wala_open", false]]);

        await setDoc(doc(collection(db, collectionName), fightId), fightData);
    };
    ...
    const handleStartNewGame = async () => {
        ...
        const fightData = {
            id: fightId,
            fight_number: 1,
            game_number: adminConfig.current_game + 1,
            meron_odds: 1,
            meron_total: 0,
            wala_odds: 1,
            wala_total: 0,
        ...
        await updateAdminConfig([["current_game", adminConfig.current_game + 1], ["current_fight", 1], ["meron_open", false], ["wala_open", false]]);
        await setDoc(doc(collection(db, collectionName), fightId), fightData);
    };
        ...
        <div className="grid grid-cols-1 md:grid-cols-10 gap-6 w-full">
            <div className="bg-gradient-to-b col-span-3 justify-center from-white via-white to-gray-400 rounded-xl shadow p-10 flex flex-col items-center">
                <div className="flex-col justify-center items-center w-full">
                    <p className="text-5xl w-full text-start font-semibold mb-1"><span className="font-light text-11xl font-mono">₱</span> {formatNumber(adminConfig.commission_total)}</p>
            ...
            <div className="bg-gradient-to-b col-span-3 from-white via-white to-gray-400 rounded-xl shadow p-10 flex flex-col items-center justify-center">
                <div className="flex-col justify-center items-center w-full">
                    <p className="text-5xl w-full text-start font-semibold mb-1">FIGHT #{adminConfig.current_fight}</p>
                    <p className="text-xl w-full text-start tracking-widest">GAME #{adminConfig.current_game}</p>
                </div>
            </div>
            ...
            <div className="bg-gradient-to-b col-span-4 from-white justify-center via-white to-gray-400 rounded-xl shadow p-6 flex flex-col items-center gap-4 row-span-2">
                <div className="w-full grid grid-cols-2 gap-2">
                    <button 
                        className="p-4 col-span-2 w-full bg-gradient-to-b from-green-400 to-green-600 hover:from-green-500 hover:to-green-700 text-white font-semibold text-2xl py-3 rounded-xl mb-2 shadow transition"
                    ...
                    <button
                        className="p-4 col-span-2 w-full bg-gradient-to-b from-green-400 to-green-600 hover:from-green-500 hover:to-green-700 text-white font-semibold text-2xl py-3 rounded-xl mb-2 shadow transition"
                        onClick={handleStartNewFight}
                    >
                        START NEW FIGHT
                    </button>
                    <button
                        className={`p-4 col-span-1 w-full bg-gradient-to-b ${adminConfig.show_display ? "from-green-400 to-green-600 hover:from-green-500 hover:to-green-700" : "from-gray-400 to-gray-600 cursor-not-allowed"} text-white font-semibold text-xl py-3 rounded-xl mb-2 shadow transition`}
                        onClick={async () => await updateAdminConfig([["show_display", !adminConfig.show_display]])}
                    >
                        TV SCREEN: {adminConfig.show_display ? "ENABLED" : "DISABLED"}
                    </button>
                    ...
                    <button
                        className={`p-4 col-span-1 w-full bg-gradient-to-b ${adminConfig.meron_open ? "from-green-400 to-green-600 hover:from-green-500 hover:to-green-700" : "from-gray-400 to-gray-600 cursor-not-allowed"} text-white font-semibold text-xl py-3 rounded-xl mb-2 shadow transition`}
                        onClick={async () => await updateAdminConfig([["meron_open", !adminConfig.meron_open]])}
                    >
                        <p>MERON:</p>
                        <p>{adminConfig.meron_open ? "ENABLED" : "DISABLED"}</p>
                    </button>
                    <button
                        className={`p-4 col-span-1 w-full bg-gradient-to-b ${adminConfig.wala_open ? "from-green-400 to-green-600 hover:from-green-500 hover:to-green-700" : "from-gray-400 to-gray-600 cursor-not-allowed"} text-white font-semibold text-xl py-3 rounded-xl mb-2 shadow transition`}
                        onClick={async () => await updateAdminConfig([["wala_open", !adminConfig.wala_open]])}
                    >
                        <p>WALA:</p>
                        <p>{adminConfig.wala_open ? "ENABLED" : "DISABLED"}</p>
                    </button>
                </div>
            </div>
            ...
            <div className="bg-gradient-to-b col-span-3 from-white via-white to-gray-400 rounded-xl shadow p-6 flex flex-col items-center">
                <div className="flex-col justify-center items-center w-full">
                    <p className="text-5xl w-full text-start font-semibold mb-1"><span className="font-light font-mono text-10xl">₱</span> {formatNumber(adminConfig.petty_cash)}</p>
                ...
                {showPettyResetModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
                        <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md">
                            ...
                            <form
                                onSubmit={async () => {await updateAdminConfig([["petty_cash", 0]]); setShowPettyResetModal(false);}}
                            >
                                <p className="mb-4">
            ...
            <div className="bg-gradient-to-b col-span-3 from-white via-white to-gray-400 rounded-xl shadow p-6 flex flex-col items-center">
                <p className="text-5xl w-full text-start font-semibold mb-1">{adminConfig.commission_pct}%</p>
