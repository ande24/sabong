...
export const BetDraw = ({ onClose }: BetDrawProps) => {
    const [user, setUser] = useState<User | null>(null); 

    const [betAmount, setBetAmount] = useState('0');

    const [betMin, setBetMin] = useState(100);
    const [betMax, setBetMax] = useState(4000);
    
    const [odds, setOdds] = useState(8.00);
    const [currentFight, setCurrentFight] = useState(123);
    ...
    const handleSubmit = async () => {
        if (parseInt(betAmount) < betMin) {
            Alert.alert('Invalid Bet', `Minimum bet amount is ${betMin}.`);
            console.log('Invalid Bet', `Minimum bet amount is ${betMin}.`);
            return;
        }
        else if (parseInt(betAmount) > betMax) {
            Alert.alert('Invalid Bet', `Maximum bet amount is ${betMax}.`);
            console.log('Invalid Bet', `Maximum bet amount is ${betMax}.`);
            return;
        }
        try {
            if (!user) {
            ...
            Alert.alert('DRAW!', 'Your bet has been placed successfully!');
            onClose();
        } catch (error) {
            ...
            >
                <View className="flex-row justify-between w-full mb-2 py-4">
                    <View className='flex-row items-center justify-center gap-x-4'>
                        <Text className="text-white text-lg">Min: {betMin}</Text>
                        <Text className="text-white text-lg">Max: {betMax}</Text>
                    </View>
                ...
                <View className="flex-row items-center justify-center mb-4 h-16">
                    <TouchableOpacity
                        ...
                        onPress={() =>
                        setBetAmount((prev) =>
                            Math.max(betMin, parseInt(prev) - 1).toString()
                        )
                        }
                    >
                        <StyledGradient
                    ...
                    <TouchableOpacity
                        className="bg-blue-600 rounded-r-md flex-[2] h-full flex items-center justify-center"
                            onPress={() =>
                            setBetAmount((prev) =>
                                Math.min(betMax, parseInt(prev) + 1).toString()
                            )
                        }
                    >
                        <StyledGradient
